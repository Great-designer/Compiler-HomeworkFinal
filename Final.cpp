...
struct instruction while_stmt()
{
	...
	while(next!=26)//右大括号 
	{
		...
		switch(next)
		{
			...
			break;
			case 7://不回退 
				struct instruction cscs=if_stmt();//break行为
				bobo=instrcat(bobo,cscs);//拼接 
				BREAKSTACK[BREAKSTACKTOPTOP-1][BREAKSTACKTOP[BREAKSTACKTOPTOP-1]].count=bobo.count;
				BREAKSTACK[BREAKSTACKTOPTOP-1][BREAKSTACKTOP[BREAKSTACKTOPTOP-1]].length=bobo.length;
			break;
			case 10://不回退 
				struct instruction cscs=while_stmt();//新循环 
				bobo=instrcat(bobo,cscs);//拼接 
				BREAKSTACK[BREAKSTACKTOPTOP-1][BREAKSTACKTOP[BREAKSTACKTOPTOP-1]].count=bobo.count;
				BREAKSTACK[BREAKSTACKTOPTOP-1][BREAKSTACKTOP[BREAKSTACKTOPTOP-1]].length=bobo.length;
			break;
			...
			case 25://block_stmt，回退 
				unreadToken();
				struct instruction cscs=block_stmt();//break行为
				bobo=instrcat(bobo,cscs);//拼接 
				BREAKSTACK[BREAKSTACKTOPTOP-1][BREAKSTACKTOP[BREAKSTACKTOPTOP-1]].count=bobo.count;
				BREAKSTACK[BREAKSTACKTOPTOP-1][BREAKSTACKTOP[BREAKSTACKTOPTOP-1]].length=bobo.length;
			break;
			case 23:
				unreadToken();
				struct instruction cscs=expr_stmt();//非break行为
				bobo=instrcat(bobo,cscs);//拼接 
				BREAKSTACK[BREAKSTACKTOPTOP-1][BREAKSTACKTOP[BREAKSTACKTOPTOP-1]].count=bobo.count;
				BREAKSTACK[BREAKSTACKTOPTOP-1][BREAKSTACKTOP[BREAKSTACKTOPTOP-1]].length=bobo.length;
			break;
