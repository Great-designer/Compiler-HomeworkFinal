	...
	else if(op==51)//标识符。这就麻烦大了，要查表决定是什么类型的变量，函数调用也在这部分，预读决定赋值也在这部分 
	{
		...
		else//不是以上两种情况，意味着就是一个普通的标识符。这个时候要把值取出来放到栈顶 
		{
			...
			if(OVERALL==0)//全局态。这时只需要查全局变量表即可 
			{
				...
				{
					if(TABLE[finding].type==0)//先检查变量类型
					{
						exit(-1);
					}
			...
			else if(OVERALL==1)//函数态。这时全都要查 
			{
				...
				{
					if(FUNCLIST[FUNCLISTTOP].localtable[finding].type==0)//先检查变量类型
					{
						exit(-1);
					}
...
struct expression combine(struct expression lhs,int op,struct expression rhs)//combine将三部分组合。这里处理语义 
{
	...
	else if(op==30)//不等于。栈顶是1或-1则为1，否则为0。跳转用是0和非0，故不用管 
	{
	...
	else if(op==34)//小于。栈顶是-1则为1，否则为0。即仅小于0的时候为1 
	{
		char nene;//比较指令。结束时结果储存在栈顶，先左小于后右是-1 
		if(lhs.type==1)
		{
	...
	else if(op==35)//大于。栈顶是1则为1，否则为0。即仅大于0的时候为1 
	{
		char nene;//比较指令。结束时结果储存在栈顶，先左小于后右是-1 
		if(lhs.type==1)
		{
	...
	else if(op==38)//小于等于。栈顶是0或-1则为1，否则为0 
	{
		char nene;//比较指令。结束时结果储存在栈顶，先左小于后右是-1 
		if(lhs.type==1)
		{
	...
	else if(op==39)//大于等于。栈顶是0或1则为1，否则为0 
	{
		char nene;//比较指令。结束时结果储存在栈顶，先左小于后右是-1 
		if(lhs.type==1)
		{
...
{
	if(peek==22||peek==32||peek==21||peek==31||peek==30||peek==34||peek==35||peek==37||peek==38||peek==39)//没有括号、赋值和as，有比较 
	{
		return 1; 
	}
	else//认为终结 
...
struct instruction while_stmt()
{
	...
	struct instruction bobo=expr().instr;//均已调整为brfalse，不满足时跳转 
...
struct instruction if_stmt()
{
	...
	int pspstop=0; 
	struct instruction fifi=expr().instr;//最终返回的instruction 
